...
#in my case there are only 2 users
...
#all 3 users can login at a single page--
#all 2 users can be singup from a single page--                    
...
def login():
    if request.method=="POST":
        ...
        if user and user.password==password:
            ...
            if not user:
                error_msg="No user Found with provided email or username"
...
def create_category(curr_login_id):
    ...
    if request.method=="POST":
        ...
        try:
            ...
            db.session.rollback()
            flash('Catgory is already exists')
...
def edit_category(curr_login_id,category_id):
    ...
    if request.method=="POST":
        try:
            ...
            db.session.commit()
            return redirect(url_for('admin_dashboard',curr_login_id=curr_login_id,category_id=category_id))
        
        except IntegrityError:
            db.session.rollback()
            flash('Catgory is already exists')
            return redirect(url_for("edit_category",curr_login_id=curr_login_id,category_id=category_id))
